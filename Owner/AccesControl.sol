...
interface IEmployeeAssignment {
    ...
    /// @param role The role string to verify.
...
abstract contract AccesControl is Initializable {
    ...
    function __AccessControl_init(address _employeeAssignment) internal onlyInitializing {
    ...
    modifier onlyEmployes() {
        require(employeeAssignment.hasRole(msg.sender, "Employe"), "Not employee");
        _;
    }

    /// @notice Restricts function access to users who do not have the "Employe" role.
    modifier notEmployes() {
        require(!employeeAssignment.hasRole(msg.sender, "Employe"), "Is employee");
        _;
    }
