...
contract EmployeeAssignment {
    ...
    modifier onlyEmployes() {
        require(employees[msg.sender]["Employe"], "EmployeeAssignment: caller is not an employee");
        _;
    }
    
    /// @notice Ensures that the caller is not an employee with Admin role
    modifier notEmployes() {
        require(!employees[msg.sender]["Employe"], "EmployeeAssignment: caller is an employee");
        _;
    }
