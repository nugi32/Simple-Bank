...
/// - Administrative actions are delegated to an external AdminLogic contract via delegatecall
/// - Access control uses AccesControl (inherited)
contract LoanData is AccesControl, ReentrancyGuardUpgradeable {
    ...
    struct Loan {
        ...
        uint principal;           // Principal in wei
        uint durationMonths;      // Total months for repayment
        uint startTimestamp;      // Timestamp when loan was disbursed
        uint paidMonths;          // Number of months paid so far
        uint consecutiveMissed;   // Counter for consecutive missed months
        uint remainingPrincipal;  // Remaining principal in wei
        uint collateral;          // Collateral held in contract (wei)
        bool liquidated;          // Whether loan has been liquidated
    ...
    /// @notice Collateral ratio required for loans (percent)
    uint16 public collateralRatioPercent;

    /// @notice Maximum Non-Performing Loan percentage allowed (percent)
    uint16 public maxNPLPercent;

    /// @notice Total profits accumulated by the protocol (wei)
    ...
    /// @notice Emitted when initial addresses are set during initialization
    ...
    /// @notice Emitted when a strike is added to a borrower
    ...
    /// @notice Revert when address(0) is passed where not allowed
    error ZeroAddress();

    /// @notice Revert when loan id is invalid
    error InvalidLoanId(uint256 id);

    /// @notice Revert when caller is blacklisted
    error Blacklisted(address user);

    /// @notice Revert when addressing not blacklisted but operation assumes it is
    error NotBlacklisted(address user);

    /// @notice Revert when caller is not borrower
    error NotBorrower();

    /// @notice Revert when loan is inactive or already liquidated
    error LoanInactiveOrLiquidated();

    /// @notice Revert when provided collateral is insufficient
    error InsufficientCollateral(uint256 required, uint256 provided);

    /// @notice Revert when contract lacks enough liquidity
    error ContractLiquidityInsufficient(uint256 required, uint256 available);

    /// @notice Revert when sent ETH doesn't match expected payment
    error PaymentValueMismatch(uint256 expected, uint256 received);

    /// @notice Revert when principal param invalid
    error InvalidPrincipal();

    /// @notice Revert when duration param invalid
    error InvalidDuration();

    /// @notice Revert when new loans are blocked by NPL rules
    error NewLoansBlocked();

    /// @notice Revert when delegatecall to AdminLogic fails
    ...
    /// @dev Called during contract deployment/upgrade. Uses OpenZeppelin initializer pattern.
    /// @param _managementWallet Address for management fees (cannot be zero)
     ...
     *      - principalInETH describes ETH amount (contract multiplies by 1 ether)
     *      - Requires caller not to be an employee/owner and not blacklisted
     *      - Uses collateralRatioPercent to determine required collateral
     * @param principalInETH Loan amount in ETH (integer)
     ...
     *      - Requires loan to be active and not liquidated
     *      - Calculates penalty for late payments
     *      - Adds interest and penalty to totalProfits
     ...
     *      - Updates consecutiveMissed and triggers internal liquidation helpers
     ...
     *      - Ensures contract has enough balance to disburse principal
     ...
     * @dev Restricted to employee role
     ...
     * @return principal Original principal amount (wei)
     ...
     * @return remainingPrincipal Remaining principal amount (wei)
     * @return collateral Collateral amount (wei)
     ...
     * @return uint Monthly due amount (wei)
     */
     ...
     * @return uint Current NPL percentage (percent)
     */
